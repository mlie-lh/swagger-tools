  ...
  <div v-if="visible" :class="['json-view-container', theme, `deep-${currentDeep}`]">
    <div :class="['json-view', length ? 'closeable' : '']" :style="{ fontSize: fontSize + 'px', lineHeight: lineHeight + 'px' }">
      ...
      <span v-if="length && iconStyle === 'circle'" class="angle" @click="toggleClose">
        <svg v-if="!innerClosed" viewBox="0 0 24 24" :fill="iconColors[0]" preserveAspectRatio="xMidYMid meet" style="
            vertical-align: middle;
            color: rgb(1, 160, 228);
            height: 1em;
            width: 1em;
        ...
        <svg v-if="innerClosed" viewBox="0 0 24 24" :fill="iconColors[1]" preserveAspectRatio="xMidYMid meet" style="
            vertical-align: middle;
      ...
      <div class="content-wrap">
        <p :class="['first-line', length > 0 ? 'pointer' : '']" @click="toggleClose">
          ...
          <span v-if="items?.length > 2" class="text-blue-500 float-right" @click.stop="genCode(items)">生成代码</span>
        </p>
...
import {Data, formatValue, getDataType, isEmptyArrayOrObject, isObjectOrArray} from "./utils"
...
export default defineComponent({
  ...
  setup(props, {slots, emit}) {
    ...
    const showGenCode = inject('showGenCode')
    const genCode = items => showGenCode(items)
    return {
      innerClosed,
      templateDeep,
      visible,
